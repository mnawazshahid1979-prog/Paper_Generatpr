...
# شاندار CSS اسٹائلنگ (سکرین کا بیک گراؤنڈ کلر تبدیل کرنے کے لیے)
st.markdown(
    """
    <style>
    @import url('https://fonts.googleapis.com/earlyaccess/jameelnoorinastaleeq.css');
    
    /* پوری سکرین اور مین ایریا کا بیک گراؤنڈ کلر (آپ یہاں اپنی مرضی کا کلر کوڈ دے سکتے ہیں) */
    ...
    /* بٹن کو اوپر اور بائیں طرف سیٹ کرنے کے لیے */
    div.stButton > button {
    ...
    /* انتہائی منفرد اور جدید سوال کارڈ ڈیزائن */
...
# ==============================================================================
# [ 🎯 ] LATEX & PANDOC BASED MATH EXPRESSION GENERATOR
# ==============================================================================
def process_complete_math_expression(paragraph, latex_expr, font_size=12):
    ...
    if not latex_expr.startswith("$"):
        latex_expr_full = f"${latex_expr}$"
    else:
        latex_expr_full = latex_expr

    try:
        with tempfile.NamedTemporaryFile(suffix='.docx', delete=False) as tmp_file:
            tmp_docx_path = tmp_file.name

        cmd = ['pandoc', '-f', 'latex', '-t', 'docx', '-o', tmp_docx_path]
        result = subprocess.run(cmd, input=latex_expr_full, text=True, encoding='utf-8', capture_output=True)
    ...
    try:
        ...
        """
        paragraph._p.get_or_add_pPr().append(parse_xml(math_xml))
        return True
    except Exception:
            ...
            if st.session_state.show_questions_flag and can_update:
                ...
                if {1, 2, 5}.issubset(st.session_state.saved_sections):
                    ...
                    def generate_paper_document(sel_cls, sel_subj, sel_unit, sel_top, inst_name, logo_file):
                        ...
                        # ورڈ فائل کے مین ہیڈرز (فونٹ سائز 20 اور سنٹر الائنڈ)
                        ...
                        for q_idx in sorted(st.session_state.section_data_records.keys()):
                            if st.session_state.section_data_records[q_idx]:
                                ...
                                if q_idx == 2:
                                    ...
                                    col_widths = [Inches(0.4), Inches(1.65), Inches(1.65), Inches(0.4), Inches(1.65), Inches(1.65)]
                                    for row_i in range(num_rows):
                                        ...
                                        # پہلا سوال (بائیں جانب)
                                        idx_item_1 = row_i * 2
                                        if idx_item_1 < total_q:
                                            r_data_1 = records[idx_item_1]                
                                            cell_num_1 = sq_table.rows[row_i].cells[0]
                                            p_n1 = cell_num_1.paragraphs[0]
                                            set_rtl_paragraph(p_n1, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                            ...
                                            expr_1 = r_data_1.get("Expression", "")
                                            q_text_1 = str(r_data_1.get("Q_Text", "")) if pd.notna(r_data_1.get("Q_Text")) else ""
                                            has_expr_1 = pd.notna(expr_1) and str(expr_1).strip() != ""

                                            if len(q_text_1) > 40 or not has_expr_1:
                                                ...
                                                if q_text_1:
                                                    run_t1 = p_t1.add_run(q_text_1)
                                                    run_t1.font.name = 'Jameel Noori Nastaleeq'
                                                    run_t1.font.size = Pt(12)
                                                    run_t1.font.rtl = True
                                                if has_expr_1:
                                                    process_complete_math_expression(p_t1, str(expr_1), font_size=12)
                                            else:
                                                p_t1 = sq_table.rows[row_i].cells[1].paragraphs[0]
                                                set_rtl_paragraph(p_t1, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                                if q_text_1:
                                                    run_t1 = p_t1.add_run(q_text_1)
                                                    run_t1.font.name = 'Jameel Noori Nastaleeq'
                                                ...
                                                cell_exp_1 = sq_table.rows[row_i].cells[2]
                                                p_e1 = cell_exp_1.paragraphs[0]
                                                set_rtl_paragraph(p_e1, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                                process_complete_math_expression(p_e1, str(expr_1), font_size=12)

                                        # दूसरा सवाल (دائیں جانب)
                                        idx_item_2 = row_i * 2 + 1
                                        if idx_item_2 < total_q:
                                            r_data_2 = records[idx_item_2]
                                            
                                            cell_num_2 = sq_table.rows[row_i].cells[3]
                                            p_n2 = cell_num_2.paragraphs[0]
                                            set_rtl_paragraph(p_n2, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                            ...
                                            expr_2 = r_data_2.get("Expression", "")
                                            q_text_2 = str(r_data_2.get("Q_Text", "")) if pd.notna(r_data_2.get("Q_Text")) else ""
                                            has_expr_2 = pd.notna(expr_2) and str(expr_2).strip() != ""

                                            if len(q_text_2) > 40 or not has_expr_2:
                                                ...
                                                if q_text_2:
                                                    run_t2 = p_t2.add_run(q_text_2)
                                                    run_t2.font.name = 'Jameel Noori Nastaleeq'
                                                    run_t2.font.size = Pt(12)
                                                    run_t2.font.rtl = True
                                                if has_expr_2:
                                                    process_complete_math_expression(p_t2, str(expr_2), font_size=12)
                                            else:
                                                p_t2 = sq_table.rows[row_i].cells[4].paragraphs[0]
                                                set_rtl_paragraph(p_t2, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                                if q_text_2:
                                                    run_t2 = p_t2.add_run(q_text_2)
                                                    run_t2.font.name = 'Jameel Noori Nastaleeq'
                                                ...
                                                cell_exp_2 = sq_table.rows[row_i].cells[5]
                                                p_e2 = cell_exp_2.paragraphs[0]
                                                set_rtl_paragraph(p_e2, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                                process_complete_math_expression(p_e2, str(expr_2), font_size=12)
                                    ...
                                    for sub_i, r_data in enumerate(records, 1):
                                        ...
                                        lq_widths = [Inches(0.75), Inches(0.5), Inches(1.0), Inches(4.25), Inches(0.5)]
                                        ...
                                        set_rtl_paragraph(p_num, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                        ...
                                        if not has_expr:
                                            ...
                                            process_complete_math_expression(p_expr, str(expr), font_size=12)
                                        ...
                                        run_marks = p_marks.add_run(f"({marks_val})")
                                    ...
                                    for sub_i, r_data in enumerate(st.session_state.section_data_records[q_idx], 1):
                                        mcq_table = doc.add_table(rows=1, cols=2)
                                        mcq_table.alignment = WD_TABLE_ALIGNMENT.CENTER
                                        mcq_table.autofit = False
                                        set_rtl_table(mcq_table)

                                        mcq_table.columns[0].width = Inches(0.5)
                                        mcq_table.columns[1].width = Inches(6.0)
                                        ...
                                        cell_num = mcq_table.rows[0].cells[0]
                                        cell_txt = mcq_table.rows[0].cells[1]

                                        cell_num.width = Inches(0.5)
                                        cell_txt.width = Inches(6.0)
                                        ...
                                        if pd.notna(expr) and str(expr).strip() != "":
                                            process_complete_math_expression(p_txt, str(expr), font_size=12)

                                        q_text_val = str(r_data.get("Q_Text", "")) if pd.notna(r_data.get("Q_Text")) else ""
                                        if q_text_val:
                                            append_rtl_text(p_txt, f"   {q_text_val}")
                                        ...
                                        if opt_a or opt_b or opt_c or opt_d:
                                            ...
                                            col_widths = [Inches(0.4), Inches(1.6), Inches(0.4), Inches(1.6), Inches(0.4), Inches(1.6), Inches(0.4), Inches(1.6)]
                                            ...
                                            for idx, (label, val) in enumerate(options_data):
                                                ...
                                                if val and str(val).strip():
                                                    ...
                                                    set_rtl_paragraph(p_val, docx.enum.text.WD_ALIGN_PARAGRAPH.LEFT)
                                                    
                                                    val_str = str(val).strip()
                                                    if "\\" in val_str or "^" in val_str or "_" in val_str or "$" in val_str:
                                                        process_complete_math_expression(p_val, val_str, font_size=12)
                                                    else:
                    ...
                    file_path_pdf = os.path.join(target_folder, f"{file_name}.pdf")
                        ...
                        if st.button("پی ڈی ایف پیپر"):
                            if not st.session_state.section_data_records:
                                ...
                                try:
                                    ...
                                    if PDF_METHOD == "comtypes":
                                        try:
                                            import comtypes.client
                                            word = comtypes.client.CreateObject("Word.Application")
                                            word.Visible = False
                                            wb = word.Documents.Open(os.path.abspath(file_path_docx))
                                            wb.SaveAs2(os.path.abspath(file_path_pdf), FileFormat=17)
                                            wb.Close()
                                            word.Quit()
                                    ...
                                    if not pdf_created and PDF_METHOD == "docx2pdf":
                                        try:
                                    ...
                                    if not pdf_created:
                                        try:
                                            subprocess.run(['libreoffice', '--headless', '--convert-to', 'pdf', file_path_docx, '--outdir', target_folder], check=True, capture_output=True)
                                            if os.path.exists(file_path_pdf):
                                                pdf_created = True
                                        except Exception:
                                            pass

                                    if pdf_created and os.path.exists(file_path_pdf):
                                        with open(file_path_pdf, "rb") as f_pdf:
                                            st.download_button(
                                                label="پی ڈی ایف فائل ڈاؤن لوڈ کریں",
                                                data=f_pdf,
                                                file_name=f"{file_name}.pdf",
                                                mime="application/pdf"
                                            )
                                    else:
                                        st.warning("پی ڈی ایف کنورژن کے لیے ورڈ درکار ہے۔ آپ ورڈ فائل ڈاؤن لوڈ کر سکتے ہیں۔")
                                        with open(file_path_docx, "rb") as f_docx:
                                            st.download_button(
                                                label="ورڈ فائل ڈاؤن لوڈ کریں",
                                                data=f_docx,
                                    ...
                                    st.error(f"پی ڈی ایف بنانے میں مسئلہ پیش آیا: {e}")