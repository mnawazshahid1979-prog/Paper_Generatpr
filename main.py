    ...
    if selected_mode and ("سنگل یونٹ" in selected_mode or "Single Unit" in selected_mode):
        ...
        if selected_unit:
            ...
            if topics:
                ...
                select_all = st.checkbox("سلیکٹ آل (Select All)", key="select_all_topics")
                selected_topics = []
                for topic in topics:
            ...
            for q_id, title_text in section_types:
                with st.expander(title_text):
                    ...
                    for idx, row in enumerate(filtered_df.iterrows(), 1):
                        ...
                        # اگر MCQ ہے تو آپشنز بھی سکرین پر دکھائیں
                        if q_id == 1:
                    ...
                    for q_idx in sorted(st.session_state.saved_sections.keys()):
                        ...
                        if records:
                            ...
                            for sub_i, r_data in enumerate(records, 1):
                                ...
                                expr = str(r_data.get("Expression", "")) if pd.notna(r_data.get("Expression")) else ""
                                q_text = str(r_data.get("Q_Text", "")) if pd.notna(r_data.get("Q_Text")) else ""
                                full_q_str = f"{expr} {q_text}".strip()
                                
                                run_qt = p_q.add_run(f"   {full_q_str}")
                                run_qt.font.name = 'Jameel Noori Nastaleeq'

                                # ایم سی کیوز کے آپشنز ورڈ فائل میں
                                if q_idx == 1:
                                    opt_a = str(r_data.get("Option_A", "")) if pd.notna(r_data.get("Option_A")) else ""
                                    opt_b = str(r_data.get("Option_B", "")) if pd.notna(r_data.get("Option_B")) else ""
                                    opt_c = str(r_data.get("Option_C", "")) if pd.notna(r_data.get("Option_C")) else ""
                                    ...
                                    if opt_a or opt_b or opt_c or opt_d:
                                        ...
                                        p_opt.paragraph_format.bidi = True
                                        run_opt = p_opt.add_run(f"   (A) {opt_a}    (B) {opt_b}    (C) {opt_c}    (D) {opt_d}")
                                        run_opt.font.name = 'Jameel Noori Nastaleeq'
